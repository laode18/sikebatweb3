...
import contractdomisiliyayasanlurah from './contracts/contractdomisiliyayasanlurah';
...
const SuratDomisiliyayasan = () => {
    ...
    try {
      ...
      for (let i = 1; i <= totalDomisiliyayasanlurahs; i++) {
        ...
        if (domisiliyayasanlurah.userId !== '') {
          savedDataIds.push(domisiliyayasanlurah.userId);
          showDataStatus[domisiliyayasanlurah.userId] = true;
        }
      }
  ...
  const handleInputChangelurah = (event) => {
    ...
    setNewDomisiliyayasanlurah((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };

  const createDomisiliyayasanlurah = async (newDomisiliyayasanlurah, domisiliyayasan) => {
    try {
      setIsLoading(true);
      const accounts = await web3.eth.getAccounts();
      const domisiliyayasanlurahCount = await contractdomisiliyayasanlurah.methods.totalDomisiliyayasanlurahs().call();
      const domisiliyayasanlurahId = parseInt(domisiliyayasanlurahCount) + 1;
      const userId = domisiliyayasan.userId;
      const namaUsaha = domisiliyayasan.namaUsaha;
      const jenisUsaha = domisiliyayasan.jenisUsaha;
      const jumlahAnggota = domisiliyayasan.jumlahAnggota;
      const jamKerja = domisiliyayasan.jamKerja;
      const alamatUsaha = domisiliyayasan.alamatUsaha;
      const domisiliyayasanlurahWithId = {
        ...newDomisiliyayasanlurah,
        domisiliyayasanlurahId: domisiliyayasanlurahId.toString(),
      ...
      await contractdomisiliyayasanlurah.methods.createDomisiliyayasanlurah(domisiliyayasanlurahWithId).send({ from: accounts[0] });
  ...
  return (
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Data Surat Domisili Yayasan/Organisasi</strong>
          </CCardHeader>
          <br />
          ...
          <FontAwesomeIcon icon={faSearch} style={{ float: 'right', marginRight: '-210px', marginTop: '7px' }} />
            ...
            <CTable style={{ marginTop: '10px', borderCollapse: 'collapse', width: '100%' }}>
              ...
              <CTableBody style={{ fontSize: '14px', textAlign: 'center', verticalAlign: "middle" }}>
                {filteredDomisiliyayasans.map((domisiliyayasan, index) => {
                  const selectedUser = users.find((user) => user.username === domisiliyayasan.userId);
                  ...
                  return (
                    <CTableRow key={index}>
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>{domisiliyayasan.namaUsaha}</CTableDataCell>
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>{domisiliyayasan.jenisUsaha}</CTableDataCell>
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>
                        <button onClick={handleOpenModal} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'blue', textDecorationLine: 'underline' }}>
                          {domisiliyayasan.letterNumber}
                        </button>
                      </CTableDataCell>

                      <Modal show={showFile} onHide={handleCloseModal} size="lg">
                        <Modal.Header closeButton>
                          <Modal.Title>{domisiliyayasan.letterNumber}</Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
                          <iframe
                            title="PDF Viewer"
                            src={`http://localhost:5000/api/uploads/${domisiliyayasan.letterNumber}`}
                            width="100%"
                            height="500px"
                            allowFullScreen
                          />
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center', width: '20%' }}>
                        <Button variant="warning" style={{ marginRight: '8px' }} onClick={() => {
                          toggleModal();
                          setEditingDomisiliyayasan(domisiliyayasan);
                        }}><CIcon icon={cilPencil} style={{ color: 'white' }} /></Button>
                        <Button style={{ marginRight: '8px' }} variant="danger" onClick={() => deleteDomisiliyayasan(index)}><CIcon icon={cilTrash} style={{ color: 'white' }} /></Button>
                        <Button
                          variant={hasSavedData.includes(domisiliyayasan.userId) ? "success" : "secondary"}
                          onClick={() => {
                            toggleData(selectedUser);
                          }}
                        >
                          <CIcon icon={showDataButtonStatus[domisiliyayasan.userId] ? cilCheck : cilFile} style={{ color: 'white' }} />
                        </Button>
    ...
    Yang bertanda tangan dibawah ini, Lurah Cibabat Kecamatan Cimahi Utara Kota Cimahi, dengan ini menerangkan bahwa:
      ...
      <tr>
        ...
        <tr>
          ...
          <td style={{ width: '2%' }}>:</td>
          <td style={{ width: '60%' }}>{domisiliyayasan.jenisUsaha}</td>
        </tr>
        <tr>
          <td style={{ paddingBottom: '5px', width: '30%', paddingLeft: '30px' }}>Jumlah Anggota</td>
          <td style={{ width: '2%' }}>:</td>
          <td style={{ width: '60%' }}>{domisiliyayasan.jumlahAnggota.toString()}</td>
        </tr>
        <tr>
          <td style={{ paddingBottom: '5px', width: '30%', paddingLeft: '30px' }}>Jam Kerja</td>
          <td style={{ width: '2%' }}>:</td>
          <td style={{ width: '60%' }}>{domisiliyayasan.jamKerja}</td>
        </tr>
        <tr>
          <td style={{ paddingBottom: '5px', width: '30%', paddingLeft: '30px' }}>Alamat</td>
          <td style={{ width: '2%' }}>:</td>
          <td style={{ width: '60%' }}>{domisiliyayasan.alamatUsaha}</td>
        </tr>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop: '-50px' }}>
      ...
      <p style={{ margin: '5px 0', fontSize: '11pt' }}><b>197512032009041001</b></p>
        ...
        <CModalFooter>
        <Form>
        <Form.Control type="text" name="userId" value={domisiliyayasan.userId} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="namaUsaha" value={domisiliyayasan.namaUsaha} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="jenisUsaha" value={domisiliyayasan.jenisUsaha} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="jumlahAnggota" value={domisiliyayasan.jumlahAnggota} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="jamKerja" value={domisiliyayasan.jamKerja} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="alamatUsaha" value={domisiliyayasan.alamatUsaha} onChange={handleInputChangelurah} hidden />
        ...
        <Button variant="primary" onClick={() => createDomisiliyayasanlurah(newDomisiliyayasanlurah, domisiliyayasan)}>Accept</Button>
      </Form>
