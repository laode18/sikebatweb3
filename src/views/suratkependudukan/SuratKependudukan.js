...
import contractkependudukanlurah from './contracts/contractkependudukanlurah';
...
const SuratKependudukan = () => {
  ...
  // Search logic
  const filteredUsers = kependudukans
  .filter(kependudukan =>
    Object.values(kependudukan).some(value =>
    ...
    try {
      ...
      for (let i = 1; i <= totalKependudukanlurahs; i++) {
        ...
        if (kependudukanlurah.userId !== '') {
          savedDataIds.push(kependudukanlurah.userId);
          showDataStatus[kependudukanlurah.userId] = true;
        }
      }
  ...
  const createKependudukanlurah = async (newKependudukanlurah, kependudukan) => {
    try {
      setIsLoading(true);
      ...
      const userId = kependudukan.userId;
  ...
  return (
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Data Surat Kependudukan</strong>
          </CCardHeader>
          <br />
        ...
        <option value={50}>50 per page</option>
          ...
          <FontAwesomeIcon icon={faSearch} style={{ float: 'right', marginRight: '-210px', marginTop: '7px' }} />
            ...
            <CTable style={{ marginTop: '10px', borderCollapse: 'collapse', width: '100%' }}>
              ...
              {filteredKependudukans.map((kependudukan, index) => {
                  const selectedUser = users.find((user) => user.username === kependudukan.userId);
                    ...
                    <CTableRow key={index}>
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>
                        <button onClick={handleOpenModal} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'blue', textDecorationLine: 'underline' }}>
                          {kependudukan.letterNumber}
                        </button>
                      </CTableDataCell>

                      <Modal show={showFile} onHide={handleCloseModal} size="lg">
                        <Modal.Header closeButton>
                          <Modal.Title>{kependudukan.letterNumber}</Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
                          <iframe
                            title="PDF Viewer"
                            src={`http://localhost:5000/api/uploads/${kependudukan.letterNumber}`}
                            width="100%"
                            height="500px"
                            allowFullScreen
                          />
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>
                        <Button variant="warning" style={{ marginRight: '8px' }} onClick={() => {
                          toggleModal();
                          setEditingKependudukan(kependudukan);
                        }}><CIcon icon={cilPencil} style={{ color: 'white' }} /></Button>
                        <Button style={{ marginRight: '8px' }} variant="danger" onClick={() => deleteKependudukan(index)}><CIcon icon={cilTrash} style={{ color: 'white' }} /></Button>
                        {/* Button Show Surat */}
                        <Button
                          variant={hasSavedData.includes(kependudukan.userId) ? "success" : "secondary"}
                          onClick={() => {
                            toggleData(selectedUser);
                          }}
                        >
                          <CIcon icon={showDataButtonStatus[kependudukan.userId] ? cilCheck : cilFile} style={{ color: 'white' }} />
                        </Button>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop: '30px' }}>
      ...
      <p style={{ margin: '5px 0', fontSize: '11pt' }}><b>197512032009041001</b></p>
        ...
        <CModalFooter>
        <Form>
        <Form.Control type="text" name="userId" value={kependudukan.userId} onChange={handleInputChangelurah} hidden />
        ...
        <Button variant="primary" onClick={() => createKependudukanlurah(newKependudukanlurah, kependudukan)}>Accept</Button>
      </Form>
    ...
    <CModalBody>
      <Form.Group as={Row}>
        ...
        <Col sm="8.5">
          ...
          <FontAwesomeIcon icon={faChevronDown} className="position-absolute" style={{ right: '22px', top: '40%', transform: 'translateY(-50%)' }} />
        </Col>
      </Form.Group>
