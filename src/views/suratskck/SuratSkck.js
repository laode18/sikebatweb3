...
import contractskcklurah from './contracts/contractskcklurah';
...
const SuratSkck = () => {
  ...
  const getSkcklurahs = async () => {
    try {
      ...
      for (let i = 1; i <= totalSkcklurahs; i++) {
        ...
        if (skcklurah.userId !== '') {
          savedDataIds.push(skcklurah.userId);
          showDataStatus[skcklurah.userId] = true;
        }
      }
  ...
  const handleInputChangelurah = (event) => {
    ...
    setNewSkcklurah((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };

  const createSkcklurah = async (newSkcklurah, skck) => {
    try {
      setIsLoading(true);
      const accounts = await web3.eth.getAccounts();
      const skcklurahCount = await contractskcklurah.methods.totalSkcklurahs().call();
      const skcklurahId = parseInt(skcklurahCount) + 1;
      const userId = skck.userId;
      const keperluan = skck.keperluan;
      const skcklurahWithId = {
        ...newSkcklurah,
        skcklurahId: skcklurahId.toString(),
        userId: userId.toString(),
        keperluan: keperluan.toString(),
      };
      await contractskcklurah.methods.createSkcklurah(skcklurahWithId).send({ from: accounts[0] });
      await getSkcklurahs();
      toggleData(false);
    } catch (error) {
      console.error(error);
  ...
  return (
                    ...
                    <CTableRow key={index}>
                      ...
                      <Modal show={showFile} onHide={handleCloseModal} size="lg">
                        <Modal.Header closeButton>
                          <Modal.Title>{skck.letterNumber}</Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
                          <iframe
                            title="PDF Viewer"
                            src={`http://localhost:5000/api/uploads/${skck.letterNumber}`}
                            width="100%"
                            height="500px"
                            allowFullScreen
                          />
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>
                        ...
                        <Button style={{ marginRight: '8px' }} variant="danger" onClick={() => deleteSkck(index)}><CIcon icon={cilTrash} style={{ color: 'white' }} /></Button>
                        <Button
                          variant={hasSavedData.includes(skck.userId) ? "success" : "secondary"}
                          onClick={() => {
                            toggleData(selectedUser);
                          }}
                        >
                          <CIcon icon={showDataButtonStatus[skck.userId] ? cilCheck : cilFile} style={{ color: 'white' }} />
                        </Button>
    ...
    <table style={{ width: '100%'}}>
      ...
      Berdasarkan Surat Keterangan dari RT 05 RW 15 Nomor: 001/01/2023 Tanggal 04 Januari 2023, Nama tersebut diatas benar warga kami dan akan membuat SKCK untuk { skck.keperluan }.
      </p>
      <p style={{ margin: '5px 0', fontSize: '11pt', marginRight: '80px', lineHeight: '25px', textAlign: 'justify', textIndent: '30px' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop: '30px' }}>
      ...
      <p style={{ margin: '5px 0', fontSize: '11pt' }}><b>197512032009041001</b></p>
        ...
        <CModalFooter>
        <Form>
        <Form.Control type="text" name="userId" value={skck.userId} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="keperluan" value={skck.keperluan} onChange={handleInputChangelurah} hidden />
        ...
        <Button variant="primary" onClick={() => createSkcklurah(newSkcklurah, skck)}>Accept</Button>
      </Form>
