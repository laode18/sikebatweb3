...
import contractbersihdiri from './contracts/contractbersihdiri';
import contractbersihdirilurah from './contracts/contractbersihdirilurah';
...
const SuratBersihdiri = () => {
    ...
    try {
      ...
      for (let i = 1; i <= totalBersihdirilurahs; i++) {
        ...
        if (bersihdirilurah.userId !== '') {
          savedDataIds.push(bersihdirilurah.userId);
          showDataStatus[bersihdirilurah.userId] = true;
        }
      }
  ...
  const handleInputChangelurah = (event) => {
    ...
    setNewBersihdirilurah((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };

  const createBersihdirilurah = async (newBersihdirilurah, bersihdiri) => {
    try {
      setIsLoading(true);
      const accounts = await web3.eth.getAccounts();
      const bersihdirilurahCount = await contractbersihdirilurah.methods.totalBersihdirilurahs().call();
      const bersihdirilurahId = parseInt(bersihdirilurahCount) + 1;
      const userId = bersihdiri.userId;
      const namaAyah = bersihdiri.namaAyah;
      const namaIbu = bersihdiri.namaIbu;
      const keperluan = bersihdiri.keperluan;
      const bersihdirilurahWithId = {
        ...newBersihdirilurah,
        bersihdirilurahId: bersihdirilurahId.toString(),
        userId: userId.toString(),
        namaAyah: namaAyah.toString(),
        namaIbu: namaIbu.toString(),
        keperluan: keperluan.toString(),
      };
      await contractbersihdirilurah.methods.createBersihdirilurah(bersihdirilurahWithId).send({ from: accounts[0] });
  ...
  return (
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Data Surat Bersih Diri</strong>
          </CCardHeader>
          <br />
          ...
          <FontAwesomeIcon icon={faSearch} style={{ float: 'right', marginRight: '-210px', marginTop: '7px' }} />
            ...
            <CTable style={{ marginTop: '10px', borderCollapse: 'collapse', width: '100%' }}>
              ...
              <CTableBody style={{ fontSize: '14px', textAlign: 'center', verticalAlign: "middle" }}>
                {filteredBersihdiris.map((bersihdiri, index) => {
                  const selectedUser = users.find((user) => user.username === bersihdiri.userId);
                  ...
                  return (
                    <CTableRow key={index}>
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>{bersihdiri.namaAyah}</CTableDataCell>
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>{bersihdiri.namaIbu}</CTableDataCell>
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>
                        <button onClick={handleOpenModal} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'blue', textDecorationLine: 'underline' }}>
                          {bersihdiri.letterNumber}
                        </button>
                      </CTableDataCell>

                      <Modal show={showFile} onHide={handleCloseModal} size="lg">
                        <Modal.Header closeButton>
                          <Modal.Title>{bersihdiri.letterNumber}</Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
                          <iframe
                            title="PDF Viewer"
                            src={`http://localhost:5000/api/uploads/${bersihdiri.letterNumber}`}
                            width="100%"
                            height="500px"
                            allowFullScreen
                          />
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center', width: '20%' }}>
                        <Button variant="warning" style={{ marginRight: '8px' }} onClick={() => {
                          toggleModal();
                          setEditingBersihdiri(bersihdiri);
                        }}><CIcon icon={cilPencil} style={{ color: 'white' }} /></Button>
                        <Button style={{ marginRight: '8px' }} variant="danger" onClick={() => deleteBersihdiri(index)}><CIcon icon={cilTrash} style={{ color: 'white' }} /></Button>
                        <Button
                          variant={hasSavedData.includes(bersihdiri.userId) ? "success" : "secondary"}
                          onClick={() => {
                            toggleData(selectedUser);
                          }}
                        >
                          <CIcon icon={showDataButtonStatus[bersihdiri.userId] ? cilCheck : cilFile} style={{ color: 'white' }} />
                        </Button>
    ...
    <table style={{ width: '100%'}}>
      ...
      <tr>
        ...
        <p style={{ margin: '5px 0', fontSize: '11pt', marginRight: '80px', lineHeight: '25px', textAlign: 'justify', textIndent: '30px' }}>
        Surat Keterangan ini diberikan kepada yang bersangkutan untuk { bersihdiri.keperluan }.
      </p>
      <p style={{ margin: '5px 0', fontSize: '11pt', marginRight: '80px', lineHeight: '25px', textAlign: 'justify', textIndent: '30px' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop: '-60px' }}>
      ...
      <p style={{ margin: '5px 0', fontSize: '11pt' }}><b>197512032009041001</b></p>
        ...
        <CModalFooter>
        <Form>
        <Form.Control type="text" name="userId" value={bersihdiri.userId} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="namaAyah" value={bersihdiri.namaAyah} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="namaIbu" value={bersihdiri.namaIbu} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="keperluan" value={bersihdiri.keperluan} onChange={handleInputChangelurah} hidden />
        ...
        <Button variant="primary" onClick={() => createBersihdirilurah(newBersihdirilurah, bersihdiri)}>Accept</Button>
      </Form>
