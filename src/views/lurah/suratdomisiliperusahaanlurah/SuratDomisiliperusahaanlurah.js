...
import contractdomisiliperusahaanuser from './contracts/contractdomisiliperusahaanuser';
...
const SuratDomisiliperusahaanlurah = () => {
...
const handleShowImages = (userId) => {
  ...
  const [newDomisiliperusahaanuser, setNewDomisiliperusahaanuser] = useState({
    userId: '', // Add default values for all fields
    namaPerusahaan: '',
    jenisPerusahaan: '',
    jamKerja: '',
  ...
  const handleInputChangeuser = (event) => {
    ...
    setNewDomisiliperusahaanuser((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };

  const createDomisiliperusahaanuser = async (newDomisiliperusahaanuser, selectedUser, domisiliperusahaanlurah) => {
    try {
      setIsLoading(true);
      const accounts = await web3.eth.getAccounts();
      const domisiliperusahaanuserCount = await contractdomisiliperusahaanuser.methods.totalDomisiliperusahaanusers().call();
      const domisiliperusahaanuserId = parseInt(domisiliperusahaanuserCount) + 1;
      const userId = domisiliperusahaanlurah.userId;
      const namaPerusahaan = domisiliperusahaanlurah.namaPerusahaan;
      const jenisPerusahaan = domisiliperusahaanlurah.jenisPerusahaan;
      const jamKerja = domisiliperusahaanlurah.jamKerja;
      const alamatPerusahaan = domisiliperusahaanlurah.alamatPerusahaan;
      const walletId = selectedUser.walletId;
      const namaDocument = "Surat Domisili Perusahaan"; 
      const domisiliperusahaanuserWithId = {
        ...newDomisiliperusahaanuser,
        domisiliperusahaanuserId: domisiliperusahaanuserId.toString(),
      ...
      await contractdomisiliperusahaanuser.methods.createDomisiliperusahaanuser(domisiliperusahaanuserWithId).send({ from: accounts[0] });
  ...
  return (
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Data Surat Domisili Perusahaan Lurah</strong>
          </CCardHeader>
          <br />
          ...
          <FontAwesomeIcon icon={faSearch} style={{ float: 'right', marginRight: '-210px', marginTop: '7px' }} />
            ...
            <CTable style={{ marginTop: '10px', borderCollapse: 'collapse', width: '100%' }}>
              ...
              <CTableBody style={{ fontSize: '14px', textAlign: 'center', verticalAlign: "middle"  }}>
                {domisiliperusahaanlurahs.map((domisiliperusahaanlurah, index) => {
    ...
    Yang bertanda tangan dibawah ini, Lurah Cibabat Kecamatan Cimahi Utara Kota Cimahi, dengan ini menerangkan bahwa:
      ...
      <tr>
          ...
          <td>:</td>
          <td>{domisiliperusahaanlurah.namaPerusahaan}</td>
        </tr>
        <tr>
          <td style={{ paddingBottom: '5px', width: '30%', paddingLeft: '30px' }}>Bidang</td>
          <td style={{ width: '2%' }}>:</td>
          <td style={{ width: '60%' }}>{domisiliperusahaanlurah.jenisPerusahaan}</td>
        </tr>
        <tr>
          <td style={{ paddingBottom: '5px', width: '30%', paddingLeft: '30px' }}>Jam Kerja</td>
          <td style={{ width: '2%' }}>:</td>
          <td style={{ width: '60%' }}>{domisiliperusahaanlurah.jamKerja}</td>
        </tr>
        <tr>
          <td style={{ paddingBottom: '5px', width: '30%', paddingLeft: '30px' }}>Alamat</td>
          <td style={{ width: '2%' }}>:</td>
          <td style={{ width: '60%' }}>{domisiliperusahaanlurah.alamatPerusahaan}</td>
        </tr>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop: '-30px' }}>
      ...
      <p style={{ margin: '5px 0', fontSize: '11pt' }}><b>197512032009041001</b></p>
        ...
        <CModalFooter>
        <Form>
        <Form.Control type="text" name="userId" value={domisiliperusahaanlurah.userId} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="namaPerusahaan" value={domisiliperusahaanlurah.namaPerusahaan} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="jenisPerusahaan" value={domisiliperusahaanlurah.jenisPerusahaan} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="jamKerja" value={domisiliperusahaanlurah.jamKerja} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="alamatPerusahaan" value={domisiliperusahaanlurah.alamatPerusahaan} onChange={handleInputChangeuser} hidden />
        ...
        <Button variant="primary" onClick={() => {
          ...
          createDomisiliperusahaanuser(newDomisiliperusahaanuser, selectedUser, domisiliperusahaanlurah);
        }}>
          TTD & Stempel
        </Button>
        </Form>
...
export default SuratDomisiliperusahaanlurah;
