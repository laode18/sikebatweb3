...
import contractkematianuser from './contracts/contractkematianuser';
...
const SuratKematianlurah = () => {
  ...
  const [newKematianuser, setNewKematianuser] = useState({
    userId: '', // Add default values for all fields
    namaAlmarhum: '',
    tanggalMeninggal: '',
    jamMeninggal: '',
  ...
  const handleInputChangeuser = (event) => {
    ...
    setNewKematianuser((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };

  const createKematianuser = async (newKematianuser, selectedUser, kematianlurah) => {
    try {
      setIsLoading(true);
      const accounts = await web3.eth.getAccounts();
      const kematianuserCount = await contractkematianuser.methods.totalKematianusers().call();
      const kematianuserId = parseInt(kematianuserCount) + 1;
      const userId = kematianlurah.userId;
      const namaAlmarhum = kematianlurah.namaAlmarhum;
      const tanggalMeninggal = kematianlurah.tanggalMeninggal;
      const jamMeninggal = kematianlurah.jamMeninggal;
      const lokasiMeninggal = kematianlurah.lokasiMeninggal;
      const penyebabMeninggal = kematianlurah.penyebabMeninggal;
      const usia = kematianlurah.usia;
      const walletId = selectedUser.walletId;
      const namaDocument = "Surat Kematian"; 
      const kematianuserWithId = {
        ...newKematianuser,
        kematianuserId: kematianuserId.toString(),
      ...
      await contractkematianuser.methods.createKematianuser(kematianuserWithId).send({ from: accounts[0] });
    ...
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Data Surat Kematian Lurah</strong>
          </CCardHeader>
          <br />
          ...
          <FontAwesomeIcon icon={faSearch} style={{ float: 'right', marginRight: '-210px', marginTop: '7px' }} />
            ...
            <CTable style={{ marginTop: '10px', borderCollapse: 'collapse', width: '100%' }}>
              ...
              <CTableBody style={{ fontSize: '14px', textAlign: 'center', verticalAlign: "middle" }}>
                {kematianlurahs.map((kematianlurah, index) => {
    ...
    <table style={{ width: '100%'}}>
      <tbody>
        <tr>
          ...
          <td style={{ width: '2%' }}>:</td>
          <td style={{ width: '60%' }}>{kematianlurah.namaAlmarhum}</td>
        </tr>
        <tr>
  ...
  <p style={{ margin: '5px 0', fontSize: '11pt', marginTop: '40px' }}>
    <u><b>Faisal, S.Si, M.A.P</b></u>
  </p>
  <p style={{ margin: '5px 0', fontSize: '11pt' }}>
    <b>197512032009041001</b>
  </p>
</div>
        ...
        <CModalFooter>
        <Form>
        <Form.Control type="text" name="userId" value={kematianlurah.userId} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="namaAlmarhum" value={kematianlurah.namaAlmarhum} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="tanggalMeninggal" value={kematianlurah.tanggalMeninggal} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="jamMeninggal" value={kematianlurah.jamMeninggal} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="lokasiMeninggal" value={kematianlurah.lokasiMeninggal} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="penyebabMeninggal" value={kematianlurah.penyebabMeninggal} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="usia" value={kematianlurah.usia.toString()} onChange={handleInputChangeuser} hidden />
        ...
        <Button variant="primary" onClick={() => {
          ...
          createKematianuser(newKematianuser, selectedUser, kematianlurah);
        }}>
          TTD & Stempel
        </Button>
        </Form>
...
export default SuratKematianlurah;
