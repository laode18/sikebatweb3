...
import contractbelumnikahuser from './contracts/contractbelumnikahuser';
...
const SuratBelumnikahlurah = () => {
  ...
  const [newBelumnikahuser, setNewBelumnikahuser] = useState({
    userId: '', // Add default values for all 
    walletId: '',
    namaDocument: '',
  });
  ...
  const handleInputChangeuser = (event) => {
    ...
    setNewBelumnikahuser((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };

  const createBelumnikahuser = async (newBelumnikahuser, selectedUser, belumnikahlurah) => {
    try {
      setIsLoading(true);
      const accounts = await web3.eth.getAccounts();
      const belumnikahuserCount = await contractbelumnikahuser.methods.totalBelumnikahusers().call();
      const belumnikahuserId = parseInt(belumnikahuserCount) + 1;
      const userId = belumnikahlurah.userId;
      const walletId = selectedUser.walletId;
      const namaDocument = "Surat Belum Nikah"; 
      const belumnikahuserWithId = {
        ...newBelumnikahuser,
        belumnikahuserId: belumnikahuserId.toString(),
        userId: userId.toString(),
        walletId: walletId.toString(),
        namaDocument: namaDocument.toString(),
      };
      await contractbelumnikahuser.methods.createBelumnikahuser(belumnikahuserWithId).send({ from: accounts[0] });
  ...
  return (
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Data Surat Belum Nikah Lurah</strong>
          </CCardHeader>
          <br />
          ...
          <FontAwesomeIcon icon={faSearch} style={{ float: 'right', marginRight: '-210px', marginTop: '7px' }} />
            ...
            <CTable style={{ marginTop: '10px', borderCollapse: 'collapse', width: '100%' }}>
              ...
              <CTableBody style={{ fontSize: '14px', textAlign: 'center', verticalAlign: "middle" }}>
                {belumnikahlurahs.map((belumnikahlurah, index) => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop: '30px' }}>
      ...
      <p style={{ margin: '5px 0', fontSize: '11pt' }}><b>197512032009041001</b></p>
        ...
        <CModalFooter>
        <Form>
        <Form.Control type="text" name="userId" value={belumnikahlurah.userId} onChange={handleInputChangeuser} hidden />
        ...
        <Button variant="primary" onClick={() => {
          ...
          createBelumnikahuser(newBelumnikahuser, selectedUser, belumnikahlurah);
        }}>
          TTD & Stempel
        </Button>
        </Form>
...
export default SuratBelumnikahlurah;
