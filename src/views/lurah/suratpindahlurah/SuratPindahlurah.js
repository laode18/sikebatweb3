...
import contractpindahuser from './contracts/contractpindahuser';
...
const SuratPindahlurah = () => {
  ...
  const [newPindahuser, setNewPindahuser] = useState({
    userId: '', // Add default values for all fields
    kotaTujuan: '',
    kelurahanTujuan: '',
    alamatTujuan: '',
  ...
  const handleInputChangeuser = (event) => {
    ...
    setNewPindahuser((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };

  const createPindahuser = async (newPindahuser, selectedUser, pindahlurah) => {
    try {
      setIsLoading(true);
      const accounts = await web3.eth.getAccounts();
      const pindahuserCount = await contractpindahuser.methods.totalPindahusers().call();
      const pindahuserId = parseInt(pindahuserCount) + 1;
      const userId = pindahlurah.userId;
      const kotaTujuan = pindahlurah.kotaTujuan;
      const kelurahanTujuan = pindahlurah.kelurahanTujuan;
      const alamatTujuan = pindahlurah.alamatTujuan;
      const rtTujuan = pindahlurah.rtTujuan;
      const rwTujuan = pindahlurah.rwTujuan;
      const alasanPindah = pindahlurah.alasanPindah;
      const tanggalPindah = pindahlurah.tanggalPindah;
      const walletId = selectedUser.walletId;
      const namaDocument = "Surat Pindah"; 
      const pindahuserWithId = {
        ...newPindahuser,
        pindahuserId: pindahuserId.toString(),
      ...
      await contractpindahuser.methods.createPindahuser(pindahuserWithId).send({ from: accounts[0] });
  ...
  return (
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Data Surat Pindah Lurah</strong>
          </CCardHeader>
          <br />
          ...
          <FontAwesomeIcon icon={faSearch} style={{ float: 'right', marginRight: '-210px', marginTop: '7px' }} />
            ...
            <CTable style={{ marginTop: '10px', borderCollapse: 'collapse', width: '100%' }}>
              ...
              <CTableBody style={{ fontSize: '14px', textAlign: 'center', verticalAlign: "middle" }}>
                {pindahlurahs.map((pindahlurah, index) => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop: '30px' }}>
      ...
      <p style={{ margin: '5px 0', fontSize: '11pt' }}><b>197512032009041001</b></p>
        ...
        <CModalFooter>
        <Form>
        <Form.Control type="text" name="userId" value={pindahlurah.userId} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="kotaTujuan" value={pindahlurah.kotaTujuan} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="kelurahanTujuan" value={pindahlurah.kelurahanTujuan} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="alamatTujuan" value={pindahlurah.alamatTujuan} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="rtTujuan" value={pindahlurah.rtTujuan} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="rwTujuan" value={pindahlurah.rwTujuan} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="alasanPindah" value={pindahlurah.alasanPindah} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="tanggalPindah" value={pindahlurah.tanggalPindah} onChange={handleInputChangeuser} hidden />
        ...
        <Button variant="primary" onClick={() => {
          ...
          createPindahuser(newPindahuser, selectedUser, pindahlurah);
        }}>
          TTD & Stempel
        </Button>
        </Form>
...
export default SuratPindahlurah;
