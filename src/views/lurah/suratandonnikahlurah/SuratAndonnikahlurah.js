...
import contractandonnikah from 'src/views/suratandonnikah/contracts/contractandonnikah';
import contractandonnikahlurah from 'src/views/suratandonnikah/contracts/contractandonnikahlurah';
import contractandonnikahuser from './contracts/contractandonnikahuser';
...
const SuratAndonnikahlurah = () => {
  ...
  const [newAndonnikahuser, setNewAndonnikahuser] = useState({
    userId: '', // Add default values for all fields
    namaPasangan: '',
    alamatPasangan: '',
    walletId: '',
  ...
  const getAndonnikahlurahs = async () => {
    try {
      setIsLoading(true);
      const totalAndonnikahlurahs = await contractandonnikahlurah.methods.totalAndonnikahlurahs().call();
      const andonnikahlurahsArray = [];

      for (let i = 1; i <= totalAndonnikahlurahs; i++) {
        const andonnikahlurah = await contractandonnikahlurah.methods.getAndonnikahlurah(i).call();
        if (andonnikahlurah.userId !== '') {
          andonnikahlurahsArray.push(andonnikahlurah);
        }
      }

      setAndonnikahlurahs(andonnikahlurahsArray);
      setIsLoading(false);
    } catch (error) {
      console.error(error);
      setIsLoading(false);
  ...
  const handleInputChangeuser = (event) => {
    ...
    setNewAndonnikahuser((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };

  const createAndonnikahuser = async (newAndonnikahuser, selectedUser, andonnikahlurah) => {
    try {
      setIsLoading(true);
      const accounts = await web3.eth.getAccounts();
      const andonnikahuserCount = await contractandonnikahuser.methods.totalAndonnikahusers().call();
      const andonnikahuserId = parseInt(andonnikahuserCount) + 1;
      const userId = andonnikahlurah.userId;
      const namaPasangan = andonnikahlurah.namaPasangan;
      const alamatPasangan = andonnikahlurah.alamatPasangan;
      const walletId = selectedUser.walletId;
      const namaDocument = "Surat Andon Nikah"; 
      const andonnikahuserWithId = {
        ...newAndonnikahuser,
        andonnikahuserId: andonnikahuserId.toString(),
        userId: userId.toString(),
        namaPasangan: namaPasangan.toString(),
        alamatPasangan: alamatPasangan.toString(),
        walletId: walletId.toString(),
        namaDocument: namaDocument.toString(),
      };
      await contractandonnikahuser.methods.createAndonnikahuser(andonnikahuserWithId).send({ from: accounts[0] });
      await getAndonnikahlurahs();
      toggleData(false);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  return (
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Data Surat Andon Nikah Lurah</strong>
          </CCardHeader>
          <br />
      ...
      <select style={{ borderRadius: '6px' }} value={itemsPerPage} onChange={(e) => setItemsPerPage(parseInt(e.target.value))}>
            ...
            <CTable style={{ marginTop: '10px', borderCollapse: 'collapse', width: '100%' }}>
              <CTableHead style={{ backgroundColor: 'grey', color: 'white', fontSize: '16px', verticalAlign: "middle", lineHeight: '20px' }}>
                <CTableRow>
                  ...
                  <CTableHeaderCell scope="col" style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>Nama Pasangan</CTableHeaderCell>
                  {/* <CTableHeaderCell scope="col" style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>Alamat Pasangan</CTableHeaderCell> */}
              ...
              <CTableBody style={{ fontSize: '14px', textAlign: 'center', verticalAlign: "middle" }}>
                {andonnikahlurahs.map((andonnikahlurah, index) => {
                  const selectedUser = users.find((user) => user.username === andonnikahlurah.userId);
                  ...
                  return (
                    <CTableRow key={index}>
                      ...
                      {/* <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'left' }}>{andonnikah.userId}</CTableDataCell> */}
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>{andonnikahlurah.namaPasangan}</CTableDataCell>
                      {/* <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'left' }}>{andonnikah.alamatPasangan}</CTableDataCell> */}
        ...
        <CModalHeader closeButton>
          <CModalTitle>Surat Keterangan Andon Nikah</CModalTitle>
        </CModalHeader>
        <Form>
        <CModalBody>
        <center>
    ...
    <div style={{ marginTop: '30px', textAlign: 'center' }}>
      <p style={{ margin: '5px 0', fontSize: '11pt', fontWeight: 'bold' }}>SURAT KETERANGAN ANDON NIKAH</p>
      <p style={{ margin: '5px 0', fontSize: '11pt' }}>NOMOR : {String(index + 1).padStart(3, '0')}.4/{nomorSurat}</p>
    </div>
    ...
    <p style={{ margin: '5px 0', fontSize: '11pt', textIndent: '30px', marginRight: '80px', lineHeight: '25px', textAlign: 'justify' }}>
    Berdasarkan Pengakuan yang bersangkutan dan Surat Keterangan dari Ketua RT.08 RW.11 Nomor : /10/2022 tanggal 11 Oktober 2022. 
    Nama tersebut di atas telah memohon Surat Keterangan Andon Nikah untuk melangsungkan pernikahan dengan seorang laki-laki
    </p>
    <table style={{ width: '100%'}}>
      <tbody>
        <tr>
          <td style={{ paddingBottom: '5px', width: '30%', paddingLeft: '30px' }}>Nama Pasangan</td>
          <td style={{ width: '2%' }}>:</td>
          <td style={{ width: '60%' }}>{andonnikahlurah.namaPasangan}</td>
        </tr>
        <tr>
          <td style={{ paddingBottom: '5px', paddingLeft: '30px' }}>Alamat</td>
          <td>:</td>
          <td>{andonnikahlurah.alamatPasangan}</td>
        </tr>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop: '30px' }}>
      ...
      <p style={{ margin: '5px 0', fontSize: '11pt' }}><b>197512032009041001</b></p>
        ...
        <CModalFooter>
        <Form>
        <Form.Control type="text" name="userId" value={andonnikahlurah.userId} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="namaPasangan" value={andonnikahlurah.namaPasangan} onChange={handleInputChangeuser} hidden />
        <Form.Control type="text" name="alamatPasangan" value={andonnikahlurah.alamatPasangan} onChange={handleInputChangeuser} hidden />
        ...
        <Button variant="primary" onClick={() => {
          ...
          createAndonnikahuser(newAndonnikahuser, selectedUser, andonnikahlurah);
        }}>
          TTD & Stempel
        </Button>
        </Form>
      ...
      <CModal visible={showModal} onClose={closeModal}>
        <CModalHeader closeButton>
          <CModalTitle>Edit Data Surat Andon Nikah</CModalTitle>
        </CModalHeader>
        {editingAndonnikah && (
        <div>
        <Form>
...
<CModal visible={showAdd} onClose={closeAdd} centered size='md'>
  <CModalHeader closeButton>
    <CModalTitle>Tambah Data Surat Andon Nikah</CModalTitle>
  </CModalHeader>
  <Form>
    <CModalBody>
      <Form.Group as={Row}>
...
export default SuratAndonnikahlurah;
