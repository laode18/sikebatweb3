...
import contractmempunyaiusahalurah from './contracts/contractmempunyaiusahalurah';
...
const SuratMempunyaiusaha = () => {
  ...
  const [newMempunyaiusahalurah, setNewMempunyaiusahalurah] = useState({
    userId: '', // Add default values for all fields
    namaUsaha: '',
    jenisUsaha: '',
    alamatUsaha: '',
    ...
    try {
      ...
      for (let i = 1; i <= totalMempunyaiusahalurahs; i++) {
        ...
        if (mempunyaiusahalurah.userId !== '') {
          savedDataIds.push(mempunyaiusahalurah.userId);
          showDataStatus[mempunyaiusahalurah.userId] = true;
        }
      }
  ...
  const handleInputChangelurah = (event) => {
    ...
    setNewMempunyaiusahalurah((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };

  const createMempunyaiusahalurah = async (newMempunyaiusahalurah, mempunyaiusaha) => {
    try {
      setIsLoading(true);
      const accounts = await web3.eth.getAccounts();
      const mempunyaiusahalurahCount = await contractmempunyaiusahalurah.methods.totalMempunyaiusahalurahs().call();
      const mempunyaiusahalurahId = parseInt(mempunyaiusahalurahCount) + 1;
      const userId = mempunyaiusaha.userId;
      const namaUsaha = mempunyaiusaha.namaUsaha;
      const jenisUsaha = mempunyaiusaha.jenisUsaha;
      const alamatUsaha = mempunyaiusaha.alamatUsaha;
      const keperluan = mempunyaiusaha.keperluan;
      const mempunyaiusahalurahWithId = {
        ...newMempunyaiusahalurah,
        mempunyaiusahalurahId: mempunyaiusahalurahId.toString(),
      ...
      await contractmempunyaiusahalurah.methods.createMempunyaiusahalurah(mempunyaiusahalurahWithId).send({ from: accounts[0] });
  ...
  return (
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Data Surat Mempunyai Usaha</strong>
          </CCardHeader>
          <br />
          ...
          <FontAwesomeIcon icon={faSearch} style={{ float: 'right', marginRight: '-210px', marginTop: '7px' }} />
            ...
            <CTable style={{ marginTop: '10px', borderCollapse: 'collapse', width: '100%' }}>
              ...
              <CTableBody style={{ fontSize: '14px', textAlign: 'center', verticalAlign: "middle" }}>
                {filteredMempunyaiusahas.map((mempunyaiusaha, index) => {
                  const selectedUser = users.find((user) => user.username === mempunyaiusaha.userId);
                  ...
                  return (
                    <CTableRow key={index}>
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>{mempunyaiusaha.namaUsaha}</CTableDataCell>
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>{mempunyaiusaha.jenisUsaha}</CTableDataCell>
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>{mempunyaiusaha.alamatUsaha}</CTableDataCell>
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>
                        <button onClick={handleOpenModal} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'blue', textDecorationLine: 'underline' }}>
                          {mempunyaiusaha.letterNumber}
                        </button>
                      </CTableDataCell>

                      <Modal show={showFile} onHide={handleCloseModal} size="lg">
                        <Modal.Header closeButton>
                          <Modal.Title>{mempunyaiusaha.letterNumber}</Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
                          <iframe
                            title="PDF Viewer"
                            src={`http://localhost:5000/api/uploads/${mempunyaiusaha.letterNumber}`}
                            width="100%"
                            height="500px"
                            allowFullScreen
                          />
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center', width: '20%' }}>
                        <Button variant="warning" style={{ marginRight: '8px' }} onClick={() => {
                          toggleModal();
                          setEditingMempunyaiusaha(mempunyaiusaha);
                        }}><CIcon icon={cilPencil} style={{ color: 'white' }} /></Button>
                        <Button style={{ marginRight: '8px' }} variant="danger" onClick={() => deleteMempunyaiusaha(index)}><CIcon icon={cilTrash} style={{ color: 'white' }} /></Button>
                        <Button
                          variant={hasSavedData.includes(mempunyaiusaha.userId) ? "success" : "secondary"}
                          onClick={() => {
                            toggleData(selectedUser);
                          }}
                        >
                          <CIcon icon={showDataButtonStatus[mempunyaiusaha.userId] ? cilCheck : cilFile} style={{ color: 'white' }} />
                        </Button>
    ...
    <table style={{ width: '100%'}}>
      ...
      <tr>
        ...
        <tr>
          ...
          <td style={{ width: '2%' }}>:</td>
          <td style={{ width: '60%' }}>{mempunyaiusaha.jenisUsaha}</td>
        </tr>
        <tr>
      ...
      Keterangan ini dibuat untuk { mempunyaiusaha.keperluan }. Surat Keterangan ini berlaku sampai dengan 05 Mei 2023. Demikian untuk dipergunakan sebagaimana mestinya.
      </p>
    </div>
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop: '30px' }}>
      ...
      <p style={{ margin: '5px 0', fontSize: '11pt' }}><b>197512032009041001</b></p>
        ...
        <CModalFooter>
        <Form>
        <Form.Control type="text" name="userId" value={mempunyaiusaha.userId} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="namaUsaha" value={mempunyaiusaha.namaUsaha} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="jenisUsaha" value={mempunyaiusaha.jenisUsaha} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="alamatUsaha" value={mempunyaiusaha.alamatUsaha} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="keperluan" value={mempunyaiusaha.keperluan} onChange={handleInputChangelurah} hidden />
        ...
        <Button variant="primary" onClick={() => createMempunyaiusahalurah(newMempunyaiusahalurah, mempunyaiusaha)}>Accept</Button>
      </Form>
