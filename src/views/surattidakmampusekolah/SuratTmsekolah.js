...
import contracttmsekolahlurah from './contracts/contracttmsekolahlurah';
...
const SuratTmsekolah = () => {
    ...
    try {
      ...
      for (let i = 1; i <= totalTmsekolahlurahs; i++) {
        ...
        if (tmsekolahlurah.userId !== '') {
          savedDataIds.push(tmsekolahlurah.userId);
          showDataStatus[tmsekolahlurah.userId] = true;
        }
      }
  ...
  const createTmsekolahlurah = async (newTmsekolahlurah, tmsekolah) => {
    try {
      setIsLoading(true);
      const accounts = await web3.eth.getAccounts();
      const tmsekolahlurahCount = await contracttmsekolahlurah.methods.totalTmsekolahlurahs().call();
      const tmsekolahlurahId = parseInt(tmsekolahlurahCount) + 1;
      const userId = tmsekolah.userId;
      const noKIP = tmsekolah.noKIP;
      const namaOrangtua = tmsekolah.namaOrangtua;
      const namaSekolah = tmsekolah.namaSekolah;
      const tmsekolahlurahWithId = {
        ...newTmsekolahlurah,
        tmsekolahlurahId: tmsekolahlurahId.toString(),
      ...
      await contracttmsekolahlurah.methods.createTmsekolahlurah(tmsekolahlurahWithId).send({ from: accounts[0] });
  ...
  return (
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Data Surat Tidak Mampu Sekolah</strong>
          </CCardHeader>
          <br />
          ...
          <FontAwesomeIcon icon={faSearch} style={{ float: 'right', marginRight: '-210px', marginTop: '7px' }} />
            ...
            <CTable style={{ marginTop: '10px', borderCollapse: 'collapse', width: '100%' }}>
              ...
              <CTableBody style={{ fontSize: '14px', textAlign: 'center', verticalAlign: "middle" }}>
                {filteredTmsekolahs.map((tmsekolah, index) => {
                  const selectedUser = users.find((user) => user.username === tmsekolah.userId);
                  ...
                  return (
                    <CTableRow key={index}>
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>{tmsekolah.noKIP}</CTableDataCell>
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>{tmsekolah.namaOrangtua}</CTableDataCell>
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>{tmsekolah.namaSekolah}</CTableDataCell>
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center' }}>
                        <button onClick={handleOpenModal} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'blue', textDecorationLine: 'underline' }}>
                          {tmsekolah.letterNumber}
                        </button>
                      </CTableDataCell>

                      <Modal show={showFile} onHide={handleCloseModal} size="lg">
                        <Modal.Header closeButton>
                          <Modal.Title>{tmsekolah.letterNumber}</Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
                          <iframe
                            title="PDF Viewer"
                            src={`http://localhost:5000/api/uploads/${tmsekolah.letterNumber}`}
                            width="100%"
                            height="500px"
                            allowFullScreen
                          />
                      ...
                      <CTableDataCell style={{ border: '1px solid black', padding: '8px', textAlign: 'center', width: '20%' }}>
                        <Button variant="warning" style={{ marginRight: '8px' }} onClick={() => {
                          toggleModal();
                          setEditingTmsekolah(tmsekolah);
                        }}><CIcon icon={cilPencil} style={{ color: 'white' }} /></Button>
                        <Button style={{ marginRight: '8px' }} variant="danger" onClick={() => deleteTmsekolah(index)}><CIcon icon={cilTrash} style={{ color: 'white' }} /></Button>
                        <Button
                          variant={hasSavedData.includes(tmsekolah.userId) ? "success" : "secondary"}
                          onClick={() => {
                            toggleData(selectedUser);
                          }}
                        >
                          <CIcon icon={showDataButtonStatus[tmsekolah.userId] ? cilCheck : cilFile} style={{ color: 'white' }} />
                        </Button>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', marginTop: '-20px' }}>
      ...
      <p style={{ margin: '5px 0', fontSize: '11pt' }}><b>197512032009041001</b></p>
        ...
        <CModalFooter>
        <Form>
        <Form.Control type="text" name="userId" value={tmsekolah.userId} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="noKIP" value={tmsekolah.noKIP} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="namaOrangtua" value={tmsekolah.namaOrangtua} onChange={handleInputChangelurah} hidden />
        <Form.Control type="text" name="namaSekolah" value={tmsekolah.namaSekolah} onChange={handleInputChangelurah} hidden />
        ...
        <Button variant="primary" onClick={() => createTmsekolahlurah(newTmsekolahlurah, tmsekolah)}>Accept</Button>
      </Form>
